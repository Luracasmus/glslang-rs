...
impl<'a> Program<'a> {
    ...
    pub fn map_io(&mut self) -> Result<(), GlslangError> {
        if unsafe { sys::glslang_program_map_io(self.handle.as_ptr()) } == 0 {
            return Err(GlslangError::MapIoError(GlslangErrorLog::new(
                self.get_log(),
                self.get_debug_log(),
            )));
        }
    ...
    pub fn link(self) -> Result<(), GlslangError> {
        ...
        {
            return Err(GlslangError::LinkError(GlslangErrorLog::new(
                self.get_log(),
                self.get_debug_log(),
            )));
        }
    ...
    pub fn get_debug_log(&self) -> String {
        let c_str = unsafe {
            CStr::from_ptr(sys::glslang_program_get_info_debug_log(
                self.handle.as_ptr(),
            ))
        };
